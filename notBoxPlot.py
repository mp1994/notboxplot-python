...
def notBoxPlot(x, xpos=1, w=0.2, ax=None, color=['#adb9e3', '#acdde7'], hatch=None, hatch_color="white", dotcolor='grey', dotsize=20, style="boxes"):
    ...
    if style == "lines":
        w = w/10
    ax.add_patch(rect(xy=(xpos-w/2, l1), width=w, height=l2, alpha=1.0, zorder=1, facecolor=color[1], ec=hatch_color, linewidth=0, hatch=hatch))  
...
if __name__ == '__main__':
    ...
    ax = notBoxPlot(x, xpos=0.75, style="lines")
    notBoxPlot(y, ax=ax, xpos=1.25, color=1, hatch="/", hatch_color="#FFAFCC")
